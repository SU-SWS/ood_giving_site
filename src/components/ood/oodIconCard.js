...
const OodIconCard = (props) => (
  <SbEditable content={props.blok}>
    <article className={`su-card su-card--link ood-icon-card ${props.blok.backgroundColor !== "white" ? "text-white" : ""}`}>
      <a href={props.blok.link} rel="nofollow noopener" className={`tile-card__link bg-${props.blok.backgroundColor}`}>
        {props.blok.icon.icon && (
          <span aria-hidden="true" className={`ood-icon-card__icon ${props.blok.icon.type} ${props.blok.icon.icon} ${props.blok.iconStyle ? props.blok.iconStyle : ""}`} />
        )}
